...
export default function DuaCard({ dua, type = 'dua', isCountingMode = false, hideAudio = false, hideCounter = false, hideTags = false, label, onDelete, hideRepeat = false }) {
    ...
    return (
        ...
        >
            ...
            <div className="flex items-center justify-between gap-4 px-6 py-4" style={{ background: t(theme, 'surface-1') }}>
                <div className="flex-1 min-w-0 pr-2">
                    {dua.reference && (
                        <span
                            className="inline-block text-[10px] sm:text-[11px] px-3 py-1.5 rounded-xl font-bold tracking-tight leading-relaxed"
                            style={{
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                    {onDelete && (
                        <button
                            ...
                            className="p-2 rounded-full transition-all duration-200 active:scale-90 opacity-60 hover:opacity-100 hover:bg-red-500/10"
                            style={{ color: '#ef4444' }}
                    ...
                    {!hideRepeat && dua.repeat && dua.repeat > 1 && (
                        <span className="text-[10px] sm:text-[11px] px-3 py-1.5 rounded-xl font-bold tracking-tight leading-relaxed tabular-nums" 
                            style={{ 
                                background: t(theme, 'surface-2'),
                                color: t(theme, 'text-primary'),
                                border: `1px solid ${t(theme, 'border')}`
                    ...
                    >
                        {fav ? <IconHeartFill size={18} /> : <IconHeart size={18} />}
                    </button>
                </div>
            </div>
        </article>
